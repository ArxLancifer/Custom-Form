...
function createTableColumns(data) {
  ...
  for (col in data) {
    descriptionsArray.push({
      title: data[col].description,
      field: data[col].description.toLowerCase(),
    });
  }
...
for (col in formData) {
  ...
  if (formData[col].type === "select") {
    myForm.append(
      createSelectComponent(formData[col].description, formData[col].option)
    );
